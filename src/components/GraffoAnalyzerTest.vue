    ...
    renderGraph() {
      ...
      const nodeSelection = graph
        ...
        .on("mouseover", (event, d) => {
          this.selectedNode = d.clave;
          highlightConnectedNodes(d.clave);
        })
        .on("mouseout", (event, d) => {
      ...
      // Resalta los nodos conectados al pasar el mouse por encima de un nodo
      function highlightConnectedNodes(node) {
        const connectedNodes = new Set();
        const connectedLinks = new Set();

        links.forEach((link) => {
          if (link.source.clave === node) {
            connectedNodes.add(link.target.clave);
            connectedLinks.add(link);
          } else if (link.target.clave === node) {
            connectedNodes.add(link.source.clave);
            connectedLinks.add(link);
          }
        });

        nodeSelection.attr("opacity", (d) =>
          connectedNodes.has(d.clave) || d.clave === node ? 1 : 0.2
        );

        linkSelection.attr("stroke-opacity", (d) =>
          connectedLinks.has(d) ? 0.6 : 0.1
        );
      }

      // Restablece los estilos de los nodos y enlaces cuando el mouse se mueve fuera de un nodo
